...
namespace lantern {
    ...
    // Go over each row and brighten the colors inside the light source
    for (let y = 0; y < halfh; y++) {
        band = this.rings;
        prev = 0;
        offset = this.offsetTable[y * this.rings + band - 1]

        // Brighten the concentric circles by remapping the colors
        while (band > 0) {
            offset = this.offsetTable[y * this.rings + band - 1]
            if (offset) {
                offset -= (Math.idiv(Math.randomRange(0, 11), 5)) // Decrease the offset to brighten
            }
