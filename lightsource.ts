...
namespace lightsource {
    ...
    export class FlashlightLightSource implements LightSource {
        ...
        // Go over each row and brighten the colors
        for (let y = -halfh; y < halfh; y++) {
            ...
            if (offset - x0 > 0) {
                ...
                changeRowLightLevel(lightMap, cx + x0, cy + y, Math.abs(x0 - offset), 255)    
            }
        }
    }
    ...
    export class CircleLightSource implements LightSource {
      ...
      for (let y = 0; y < halfh; y++) {
          ...
          // Brighten each concentric circle by remapping the colors
          while (band > 0) {
              offset = this.offsetTable[y * this.rings + band - 1]
              if (offset) {
                  offset -= (Math.idiv(Math.randomRange(0, 11), 5))
              ...
              changeRowLightLevel(lightMap, cx + offset, cy + y + 1, prev - offset, 255)
              changeRowLightLevel(lightMap, cx - prev, cy + y + 1, prev - offset, 255) 
              changeRowLightLevel(lightMap, cx + offset, cy - y, prev - offset, 255)
              changeRowLightLevel(lightMap, cx - prev, cy - y, prev - offset, 255)

              if (band == 1) {
                  changeRowLightLevel(lightMap, cx, cy + y + 1, prev, 255)
                  changeRowLightLevel(lightMap, cx-prev, cy + y + 1, prev, 255)  
                  changeRowLightLevel(lightMap, cx, cy - y, prev, 255)
                  changeRowLightLevel(lightMap, cx-prev, cy - y, prev, 255)   
              }
